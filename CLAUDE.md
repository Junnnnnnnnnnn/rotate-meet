...
- **Event sessions are DB-driven**: `Step1` fetches active sessions from `GET /api/sessions` (client `useEffect`, with loading/empty/error states) — it is *not* a hardcoded array anymore. Sessions live in the `event_sessions` table, managed by operators via the Telegram `/date` command (`/date` lists + delete/activate buttons; `/date add <date> <venue> <time>` inserts). `submit/route.ts` validates `event_session_id` against the table (must exist and be `is_active`) and denormalizes a human label into `signups.event_session_label` so notifications don't show the raw slug. All session date/time/slug formatting lives in `lib/sessions.ts` (shared by the form API, submit, webhook, and `format-notification.ts`). Deleting a session via `/date` with no signups removes the row; with signups it prompts for confirmation, then **reject-cleans each signup** (delete R2 photos, delete the chat photo messages, refresh the notify message to "✗ 세션 취소됨" with no buttons, null photo keys, set `photos_purged_*`) and sets `status='cancelled'` — *not* `'blocked'`. `cancelled` is excluded from both `signups_phone_active_uidx` (`status <> 'cancelled'`) and the blocked check (`status='blocked'`), so those people can re-sign-up for another session with the same phone — a clean reset despite the full cleanup. `buildButtons` and `formatNotification` treat `cancelled` like `blocked` (empty keyboard / cancelled status line). Migration `db/migrations/2026-05-19-event-sessions.sql` must be run manually in Supabase.
